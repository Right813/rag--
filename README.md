# 知库智答 · 企业级医疗智能知识库与 Hybrid RAG

这是一个面向医疗知识的企业级文档知识库与 Hybrid RAG 平台，按 `README_project2_RAG_KG_requirements.md` 的主链路实现：

```text
文档解析 → Structure-aware Chunk → BGE-M3 → Milvus Dense/BM25 → Hybrid Search → Reranker → Context → LLM → Citation/No-answer
```
...
- Milvus Standalone 2.4+：Dense、BM25 Sparse 和 Hybrid Search
- etcd、MinIO：Milvus Standalone 的元数据和对象存储依赖
- Neo4j Community 5：可选知识图谱和 Cypher 检索
- MySQL `127.0.0.1:3306`：会话、消息、反馈持久化，默认数据库名为 `rag_kg`；生产环境也支持 PostgreSQL
...
浏览器打开 <http://localhost:8000>，API 文档在 <http://localhost:8000/docs>，Milvus 在 `127.0.0.1:19530`，Neo4j Browser 在 <http://localhost:7474>。

### API 和依赖一起用 Docker 启动

本机 MySQL、Redis 通过 `host.docker.internal` 访问，Milvus、Neo4j 使用 Compose 内部网络：
...
- 必需：Milvus 2.4+、etcd、MinIO（向量索引和 Hybrid Search）。
- 可选：Neo4j 5（内置医疗示例和 Graph RAG 增强能力）。
- 可选：Sentence Transformers、PaddleOCR/PaddlePaddle，安装 `requirements-optional.txt` 启用 BGE-M3、Cross-Encoder 和 OCR。
...
- 默认使用 BGE-M3；模型不可用时使用确定性 Hash Dense 作为降级，BM25 稀疏向量仍写入 Milvus。
...
- 支持单文件、批量导入、增量 upsert、版本切换、按文档重新索引和删除原文件、切片及检索索引。
- 文档查询支持部门关键词搜索和访问级别过滤；生产环境请设置强随机 `ADMIN_TOKEN`。

### API

- `POST /api/v1/documents/upload`：上传并索引企业文档。
- `POST /api/v1/documents/batch-upload`：批量上传并索引文档。
- `GET /api/v1/documents`、`GET /api/v1/documents/stats`：查询文档列表和索引统计。
- `POST /api/v1/documents/{document_id}/reindex`、`POST /api/v1/documents/{document_id}/activate`、`DELETE /api/v1/documents/{document_id}`：重建索引、切换版本或删除文档。
- `POST /api/v1/retrieval/search`：直接查看 Dense、BM25 或 Hybrid Retrieval 结果和分数。
...
Milvus 客户端写在 `requirements.txt`；BGE-M3、Cross-Encoder 和 OCR 写在 `requirements-optional.txt`，按需安装：

```powershell
pip install -r requirements-optional.txt
```

文档相关 `.env` 变量包括 `DOCUMENT_STORAGE_PATH`、`DOCUMENT_UPLOAD_DIR`、`MAX_UPLOAD_SIZE_BYTES`、`EMBEDDING_MODEL`、`EMBEDDING_DIMENSION`、`RERANKER_MODEL`、`DENSE_TOP_K`、`BM25_TOP_K`、`RETRIEVAL_TOP_K`、`FUSION_STRATEGY`、`FUSION_ALPHA`、`MAX_CONTEXT_TOKENS`、`NO_ANSWER_THRESHOLD`、`OCR_ENABLED` 和 `MILVUS_*`。真实 API Key 只放在本地 `.env`，不要提交到 Git。

### 评测

使用 `data/evaluation/sample_questions.json` 作为格式样例运行离线检索评测。评测会使用当前 Milvus Hybrid Search 和 Reranker 链路：
...
脚本输出查询数量、Recall@5、Recall@10、MRR、NDCG@5 和 NDCG@10；消融脚本对比 Dense、BM25、Hybrid、Hybrid+Reranker 和 Query Rewrite。评测集可为需要改写的多轮问题增加 `rewritten_query` 字段，RAG 脚本额外输出 Faithfulness、Answer Relevance、Answer Completeness、Citation Accuracy 和 No-answer Accuracy。样例结果不代表生产数据，生产环境应使用真实标注集替换样例。
